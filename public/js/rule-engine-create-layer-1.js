...
function addnewtable() {
    ...
    sepHtml += '<th width="95%" id="tableHeadRow_' + tableno + '" class="tblHeadRow" colspan="8">';
    ...
    sepHtml += '<option value="">Bracket</option>';
    ...
    sepHtml += '<span id="addrulebutton_' + tableno + '_' + idCount + '" class="btn--primary" type="button" onclick="addMoreRuleForTableNum(' + tableno + ')">';
    ...
    sepHtml += '<tfoot>';
    /*
    sepHtml += '<tr id="addMoreRulebutton_' + tableno + '">';
...
function addMoreRuleForTableNum(tablenumber) {
    ...
    if ($('#operator_' + tableno + '_' + rowCount).val() == '') {
        ...
        var whenAddMorTableType = 'Fields';//$('#tableType_' + tablenumber).val();
...
function addMoreRule(tablnum) {
    ...
    var newHtmlText28 = newHtmlText28.replace("addrulebutton_" + tableno + "_" + rowCount, "addrulebutton_" + tableno + "_" + nextRow);
...
function addFieldOrFunction() {
    ...
    var tablenum = tableno = parseInt($('#tableNumberForAdd').val());
...
function changeCustomObjectField() {
    var ruleTypeSelectedFromPage1=$("#ruleTypeAtPage1").val();
    var customObjectsSelected=$("#customObjects").val();
    if(typeof ruleTypeSelectedFromPage1!='undefined' && ruleTypeSelectedFromPage1!='' && typeof customObjectsSelected!='undefined' && customObjectsSelected!='' ){
    $("#ruleType").val(ruleTypeSelectedFromPage1);
    ...
    $.ajax({
        ...
        alert('Please Select NameSpace First')
        $("#ruleTypeAtPage1").val('');
        return false;
    }     
    ...
    if ($("#addrule").valid()) {
        ...
        $.ajax({
            ...
            if (resultRes.status == 200) {
                ...
                //element.html(JSON.stringify(resultRes, undefined, 2));
                $("#errorMsg").html(JSON.stringify(resultRes, undefined, 2));
                //$("#errorMsg").html("Message:"resultRes.message.message+" Condition:"+resultRes.condition+" Action:"+resultRes.action);
                $('html, body').animate({ scrollTop: $('.navbar-expand-md').offset().top }, 'slow');
                return false;
            }
...
function addMoreRulethenNew(tablenum,rowCount) {
   ...
   // let rowCount = parseInt($('#totalRulesThen_' + tablenum).val());
    ...
    $("#customConditionNewTableParentLevel_" + tablenum + " tbody").append(newHtmlText9);
...
function changeCartStatementObjectFieldThenNew(obj) {
    ...
    if (selectedOptionThenmodel == 'Custom Code') {
        ...
        if (fieldJson == '' && selectedOptionThenmodel == 'Fields') {
            ...
            $('#browt_AllFunctionThenNew_' + tableno + '_' + obj).empty();
            $('#browt_AllFunctionThenNew_' + tableno + '_' + obj).append("<option value=''>");
            $.each(PClevel.jupiterMetaData.result.methodsList, function (i, item) {
                console.log('NEW ITEMS==', item);
                //  var name = "( " + item.id + "_" + item.frameType + " )"
                $('#browt_AllFunctionThenNew_' + tableno + '_' + obj).append('<option value="' + item + '">' + item + '</option>');
            });
    ...
    $('.templateVariableSymbolDataThen_' + tableno + '_' + obj).removeClass('hide');
    $('.templateVariableSymbolDataThen_' + tableno + '_' + obj).hide();
}
...
function addMoreRuleForCustomOperationsFieldLevel(tableno,currentTableSuffix) {
   // alert('tableno IN FUN'+tableno)
    ...
    if(currentTableSuffix!=''){
            ...
            //alert('allChooseFilelds');
            console.log('allChooseFilelds===',allChooseFilelds)
            if (allChooseFilelds.length > 0) {
                //New Table Then part 
...
function removeRuleRow(tablenumber, rownumber) {
    if (confirm('Are you sure to remove this record')) {
        //alert('#tableRow_' + tablenumber + '_' + rownumber);
...
function fieldModelPopUpInbetweenThenNew(tableNumber) {
    ...
    //alert("then option to add --- "+functionCallOncart);
    var obj = 0; //parseInt($("#indexValue").val()-1);
    var tableno = parseInt(tableNumber);
...
function openCustomCodeFieldLevelThenOLD(tablenum) {
...
function setvalueForTemplateThen(tableno, rowno) {
    ...
    var valueId = "ruleValueThen_"+tableno+"_"+rowno;
    if (templateDropValue == 'Variable') {
...
function setvalueForTemplateChildThen(tableSufix) {
    ...
    var valueId = "ruleValueThen_"+tableSufix;
    if (templateDropValue == 'Variable') {
...
//Different functions for different select drop down values

//function for setting field
...
function changeFieldLevelsOptions() {
    ...
    if (valueSelected == 'Function Call') {
       // alert('currentTableSuffix=='+currentTableSuffix)
        $("#tableHeadRow_" + currentTableSuffix).attr('colspan', 7);
    } else {
        $("#tableHeadRow_" + currentTableSuffix).attr('colspan', 6);
    }
}
...
function createInnertableCustomObject(tablenum,currentTableSuffix) {
    ...
    sepHtml += '<th id="tableHeadRow_' + tableIdSufix + '" class="tblHeadRow" colspan="7" width="95%">';
    ...
    sepHtml += '<div class="col-lg-4">';
    sepHtml += '<input style="width:250px;" list="browt_customOprFieldName_' + tableIdSufix + '" id="cusOperationAppliedOnField_' + tableIdSufix + '" name="cusOperationAppliedOnFieldChild[]" class="form-control required">';
...
function setValueForChildCustomObject(tablenum, childTableNumber, root) {
   ...
   //setting values incase custom operation selected in child
    $('#browt_customOprFieldName_' + tablenum + '_' + childTableNumber).empty();
    $('#browt_customOprFieldName_' + tablenum + '_' + childTableNumber).append("<option value=''>");
    $.each(allChooseFilelds, function (i, item) {

        $('#browt_customOprFieldName_' + tablenum + '_' + childTableNumber).append('<option value="' + item + '">' + item + '</option>');
    });

   //setting values incase custom operation selected in child ends
...
function setTemplateTypeConditions() {
    ...
    if (ruleTypeSelected == 'NORMAL' || ruleTypeSelected == 'FACT_NORMAL') {
        ...
        $('.templateTypeOption').hide();
        $('.templateTypeOptionThen').addClass('hide');
        $('.templateTypeOptionThen').hide();
        $('.templateTypeOptionThenNew').addClass('hide');
        $('.templateTypeOptionThenNew').hide();
        ...
        $('.templateTypeOption').show();
        $('.templateTypeOptionThen').removeClass('hide');
        $('.templateTypeOptionThen').show();
        $('.templateTypeOptionThenNew').removeClass('hide');
        $('.templateTypeOptionThenNew').show();
...
function addMoreRulethenChildEx(tablenum,rowCount) {
    ...
    let lastRow = $("#"+rowId).clone()
    //let lastRow = $("#tableaddRecorderThenChild_" + tablenum + " tbody tr").last().clone();
    var newHtmlText1 = '<tr class="nonCustmOprFieldForThenChild_' + tablenum + '" id="' + newRowId + '">' + lastRow.html().replace("ChoosenFieldThenChild_" + tablenum + '_' + rowCount, "ChoosenFieldThenChild_" + tablenum + '_' + nextRow) + '</tr>';
    var newHtmlText2 = newHtmlText1.replace("thheadSecondaryThen_" + tablenum + '_' + rowCount, "thheadSecondaryThen_" + tablenum + '_' + nextRow);
    var newHtmlText3 = newHtmlText2.replace("browt_ChooseFieldThen_" + tablenum + '_' + rowCount, "browt_ChooseFieldThen_" + tablenum + '_' + nextRow);
    var newHtmlText4 = newHtmlText3.replace("removeThen_" + tablenum + '_' + rowCount, "removeThen_" + tablenum + '_' + nextRow);
    var newHtmlText4 = newHtmlText4.replace("addrulebuttonthen_" + tablenum + '_' + rowCount, "addrulebuttonthen_" + tablenum + '_' + nextRow);
    ...
    $("#ChoosenFieldThen_" + tablenum + nextRow).val('');
    $(".templateVariableSymbolDataThen_" + tablenum +'_'+ nextRow).hide();
    $("#ruleValueThen_" + tablenum + nextRow).val('');
    if (nextRow > 0) {
        $("#removeThen_" + tablenum + "_" + rowCount).show();
        $("#addrulebuttonthen_" + tablenum + "_" + rowCount).remove();
    }
...
function addMoreRulethenNewChild(tablenum,rowCount) {
    ...
    $("#customConditionNewTable_" + tablenum).append(newHtmlText9);

    var nxtRl = parseInt(nextRow) + 1;
    $("#totalRulesThen_" + tablenum).val(nxtRl)

    $("#ChooseFieldThenNew_" + tablenum + nextRow).val('');
    $(".templateVariableSymbolDataThenNew_" + tablenum +'_'+ nextRow).hide();
    if (nextRow > 0) {
        $("#removeThenNew_" + tablenum + "_" + rowCount).show();
        $("#addrulebuttonthenNew_" + tablenum + "_" + rowCount).remove();
